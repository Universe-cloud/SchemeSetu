...
with tab3:
    ...
    st.write(t('chatbot_sub'))
    user_q = st.text_input(t('chatbot_input_label'), key="tab3_chatbot_input_key")
    if user_q:
        st.chat_message("user").write(user_q)
        st.chat_message("assistant").write("Based on official government guidelines, marginalized entrepreneurs are entitled to primary eligibility...")

with tab4:
    st.subheader(f"❓ {t('tab_faqs_title')}")
...
with tab5:
    st.subheader(f"🔔 {t('tab_notifications_title')}")
    email_or_phone = st.text_input(t('alerts_input_label'), key="tab5_alterts_input_key")
    if st.button(t('subscribe_btn')):
        st.success("Subscribed successfully! You will receive instant notifications when new schemes match your profile.")